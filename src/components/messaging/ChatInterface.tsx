...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ conversation, onClose }) => {
  ...
  const handleSendMessage = async (content: string, type: MessageType) => {
    ...
    try {
      ...
      setMessages(prev =>
        prev.map(msg => (msg.id.startsWith('temp-') ? { ...msg, status: 'failed' } : msg)),
      );
    } finally {
      setSending(false);
    }
  ...
  const handleSendFile = async (file: File, content?: string) => {
    ...
    try {
      ...
      const response = await messagingService.sendFileMessage(conversation.id, file, content);
  ...
  useEffect(() => {
    const unsubscribe = onMessageRead(event => {
      if (event.conversation_id === conversation.id) {
        setMessages(prev =>
          prev.map(msg =>
            msg.id === event.message_id ? { ...msg, status: 'read', read_at: event.read_at } : msg,
          ),
        );
      }
    });
