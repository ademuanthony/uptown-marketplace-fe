...
export default function ProductDetailPage() {
  ...
  const { user } = useAuth();
  const productIdOrPermalink = params.id as string;
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (user) {
          try {
            ...
            // Don't show error to user, just default to not favorited
    ...
    if (productIdOrPermalink) {
      fetchProduct();
    }
  }, [productIdOrPermalink, router, user]);
