...
class LoyaltyService {
  ...
  async getPointBalance(): Promise<LoyaltyPointsBalance> {
    try {
      const response = await api.get<LoyaltyApiResponse<LoyaltyPointsBalance>>('/loyalty/balance');
  ...
  async getLoyaltyAccount(): Promise<LoyaltyAccount> {
    try {
      const response = await api.get<LoyaltyApiResponse<LoyaltyAccount>>('/loyalty/account');
      ...
      if (status === 404) {
        ...
        try {
          ...
          const retryResponse =
            await api.get<LoyaltyApiResponse<LoyaltyAccount>>('/loyalty/account');
  ...
  async createLoyaltyAccount(): Promise<void> {
    try {
      const response = await api.post<LoyaltyApiResponse<{ user_id: string }>>('/loyalty/account');
  ...
  async checkSufficientPoints(requiredPoints: number): Promise<boolean> {
    try {
      const response = await api.post<
        LoyaltyApiResponse<{ sufficient: boolean; required_points: number }>
      >('/loyalty/check', { points: requiredPoints });
  ...
  async addPoints(points: number, description: string, referenceId?: string): Promise<void> {
    try {
      const response = await api.post<
        LoyaltyApiResponse<{ points_added: number; description: string }>
      >('/loyalty/add', {
        points,
        description,
